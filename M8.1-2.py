import sqlite3
conn = sqlite3.connect('M8.sqlite')
cursor = conn.cursor()
...
students_python_spb = cursor.execute("""SELECT Students.name, Courses.name, Students.city 
                                   FROM Students, Courses, Student_Courses 
                                   WHERE (Student_Courses.courses_id=Courses.id) and (Students.id=Student_Courses.student_id) and (Students.city='Spb') and (Courses.id = 1)""").fetchall()
print(students_30)
